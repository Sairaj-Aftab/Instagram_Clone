...
const CreatePostModal = ({ close }) => {
  const [showCreatePostModalCrop, setShowCreatePostModalCrop] = useState(false);
  const [allFile, setAllFile] = useState([]);

  const handleChangeFiles = (e) => {
    setAllFile((prev) => [...prev, ...Array.from(e.target.files)]);
    setShowCreatePostModalCrop(true);
  };
        ...
        <div className="create-post-modal">
          <div className="box">
            ...
            <div className="body">
              ...
              {showCreatePostModalCrop && allFile.length > 0 && (
                <CreatePostModalCrop
                  photos={allFile}
                  close={() => setShowCreatePostModalCrop(false)}
                  closePrev={close}
                />
              )}
            </div>
